...
namespace TsMap.TsItem
{
    public class TsCityItem : TsItem // TODO: Add zoom levels/range to show city names and icons correctly
    {
        ...
        public override string ToString()
        {
            ...
            var cityName = Sector.Mapper.Localization.GetPreferredLocaleValue(City.LocalizationToken);
            if (string.IsNullOrWhiteSpace(cityName))
            {
                cityName = !string.IsNullOrWhiteSpace(City.Name)
                    ? City.Name
                    : ScsToken.TokenToString(City.Token);
            }
