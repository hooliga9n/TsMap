...
namespace TsMap.TsItem
{
    public class TsCompanyItem : TsItem
    {
        ...
        public void TsCompanyItem825(int startOffset)
        {
            ...
            PrefabUid = MemoryHelper.ReadUInt64(Sector.Stream, fileOffset += 0x08 + 0x08); // 0x08(_companyNameToken) + 0x08(city_name)

            Nodes = new List<ulong>(1)
            {
                MemoryHelper.ReadUInt64(Sector.Stream, fileOffset += 0x08) // 0x08(PrefabUid)
            };
        ...
        public void TsCompanyItem903(int startOffset)
        {
            ...
            PrefabUid = MemoryHelper.ReadUInt64(Sector.Stream, fileOffset += 0x05 + 0x08); // 0x05(flags) + 0x08(linked_city_name)

            _companyNameToken = MemoryHelper.ReadUInt64(Sector.Stream, fileOffset += 0x08); // 0x08(PrefabUid)
        ...
        {
            if (!Sector.Mapper.AllPrefabsByUid.TryGetValue(PrefabUid, out var prefab))
            {
            ...
            if (companyPos == null)
            {
                ...
                if (companyNode == null)
                {
                    Logger.Instance.Error($"Could not find company spawn point and company node for prefab UID: 0x{PrefabUid}.");
                    return;
                }
            ...
            if (_companyNameToken == 0) return; // nameless secondary/linked company node - nothing to render
