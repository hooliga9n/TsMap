namespace TsMap.FileSystem.Hash
{
    /// <summary>
    /// Decodes SII files that use SCS's "3nK" obfuscation scheme.
    /// Some newer/large mods (and some base game files) store their .sii files
    /// this way instead of as plain text. The archive metadata reports these
    /// entries as NOT compressed, so they come out of HashEntryV2.Read() as
    /// raw obfuscated bytes unless we detect and decode them here.
    ///
    /// Format: 4-byte signature (0x33 0x6E 0x4B 0x01, i.e. "3nK" + 0x01),
    /// 1 unknown/reserved byte, 1 seed byte, followed by the payload where
    /// payload[i] is XORed with KeyTable[(seed + i) &amp; 0xFF].
    ///
    /// Algorithm ported from the open-source reference implementation:
    /// https://github.com/TheLazyTomcat/3nK_Transcode (MPL 2.0)
    /// </summary>
    internal static class Sii3NkDecoder
    {
        private const byte Sig0 = 0x33; // '3'
        private const byte Sig1 = 0x6E; // 'n'
        private const byte Sig2 = 0x4B; // 'K'
        private const byte Sig3 = 0x01;
        private const int HeaderSize = 6;

        private static readonly byte[] KeyTable =
        {
            
        };

        /// <summary>
        /// Returns true if the given buffer starts with the 3nK signature.
        /// </summary>
        internal static bool Is3NkEncoded(byte[] data)
        {
            return data != null && data.Length >= HeaderSize &&
                   data[0] == Sig0 && data[1] == Sig1 && data[2] == Sig2 && data[3] == Sig3;
        }

        /// <summary>
        /// Decodes a 3nK-encoded buffer. Assumes Is3NkEncoded(data) is true.
        /// </summary>
        internal static byte[] Decode(byte[] data)
        {
            var seed = data[5];
            var payloadLength = data.Length - HeaderSize;
            var result = new byte[payloadLength];
            for (var i = 0; i < payloadLength; i++)
            {
                result[i] = (byte) (data[HeaderSize + i] ^ KeyTable[(byte) (seed + i)]);
            }
            return result;
        }

        /// <summary>
        /// Decodes the buffer if it is 3nK-encoded, otherwise returns it unchanged.
        /// </summary>
        internal static byte[] DecodeIfNeeded(byte[] data)
        {
            return Is3NkEncoded(data) ? Decode(data) : data;
        }
    }
}
