            ...
            for (var i = 0; i < itemCount; i++)
            {
                ...
                switch (type)
                {
                    ...
                    case TsItemType.Road:
                    {
                        ...
                        lastOffset += item.BlockSize;
                        if (item.Valid) Mapper.AllRoads.Add((TsRoadItem) item); // incl. hidden, used by the RouteAdvisor export
                    ...
                    case TsItemType.Prefab:
                    {
                        ...
                        if (item.Valid)
                        {
                            var prefabItem = (TsPrefabItem) item;
                            if (!item.Hidden) Mapper.Prefabs.Add(prefabItem);
                            if (!Mapper.AllPrefabsByUid.ContainsKey(prefabItem.Uid))
                                Mapper.AllPrefabsByUid.Add(prefabItem.Uid, prefabItem);
                        }
                        break;
                    }
                    ...
                    case TsItemType.Company:
                    {
                        ...
                        if (item.Valid) Mapper.Companies.Add((TsCompanyItem) item);
                        lastOffset += item.BlockSize;
                        break;
                    }
