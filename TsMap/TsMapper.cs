...
namespace TsMap
{
    public class TsMapper
    {
        ...
        /// <summary>Name of the map taken from the .mbd file ("usa", "europe", ...). Used as the RouteAdvisor export prefix.</summary>
        ...
        /// <summary>
        /// All valid road items, INCLUDING ones marked Hidden (which are left out of <see cref="Roads"/>
        /// so they aren't drawn). The RouteAdvisor export needs them because hidden roads are still
        /// part of the drivable network.
        /// </summary>
        public readonly List<TsRoadItem> AllRoads = new List<TsRoadItem>();
        public readonly List<TsPrefabItem> Prefabs = new List<TsPrefabItem>();
        /// <summary>
        /// All valid prefab items keyed by Uid, INCLUDING ones marked Hidden (which are
        /// deliberately excluded from <see cref="Prefabs"/> so they aren't drawn on the map).
        /// Other items (e.g. companies) can still reference a hidden prefab to work out
        /// their own position, so they should look it up here instead of in Prefabs.
        /// </summary>
        public readonly Dictionary<ulong, TsPrefabItem> AllPrefabsByUid = new Dictionary<ulong, TsPrefabItem>();
        ...
        private void ParseCityFiles()
        {
            ...
            foreach (var cityFileName in defDirectory.GetFiles("city"))
            {
                ...
                if (cityFile?.Entry == null)
                {
                    Logger.Instance.Warning($"Skipping unreadable city file 'def/{cityFileName}' (likely missing from a mod .scs part or the file list is out of date)");
                    continue;
                }

                var data = cityFile.Entry.Read();
                var lines = SiiHelper.GetLines(data);
        ...
        private void ParseCountryFiles()
        {
            ...
            foreach (var countryFilePath in defDirectory.GetFiles("country"))
            {
                ...
                if (countryFile?.Entry == null)
                {
                    Logger.Instance.Warning($"Skipping unreadable country file 'def/{countryFilePath}' (likely missing from a mod .scs part or the file list is out of date)");
                    continue;
                }
        ...
        private void ParsePrefabFiles()
        {
            ...
            foreach (var prefabFileName in worldDirectory.GetFiles("prefab"))
            {
                ...
                if (prefabFile?.Entry == null)
                {
                    Logger.Instance.Warning($"Skipping unreadable prefab file 'def/world/{prefabFileName}' (likely missing from a mod .scs part or the file list is out of date)");
                    continue;
                }

                ParsePrefabFileContent(prefabFile.Entry.Read());
            }
        }

        /// <summary>
        /// Parses prefab_model definitions out of a prefab def file's raw content.
        /// If the file contains "@include" directives (used by some mods to split
        /// extra prefab definitions into separate files), those files are loaded
        /// and parsed too, recursively.
        /// </summary>
        private void ParsePrefabFileContent(byte[] data)
        {
            var lines = SiiHelper.GetLines(data);
            ...
            foreach (var line in lines)
            {
                ...
                if (line.Contains("@include"))
                {
                    var includePath = PathHelper.GetFilePath(line.Split('"')[1], "def/world");
                    var includeFile = UberFileSystem.Instance.GetFile(includePath);
                    if (includeFile?.Entry == null)
                    {
                        Logger.Instance.Warning($"Could not resolve @include '{includePath}' from a prefab file");
                        continue;
                    }
                    ParsePrefabFileContent(includeFile.Entry.Read());
                    continue;
                }
                ...
                if (validLine)
                {
                    if (key == "prefab_model")
                    {
                        var rawSuffix = value.Split('.').Last();
                        var cleanSuffix = rawSuffix.Trim().TrimEnd('{').Trim();
                        token = ScsToken.StringToToken(cleanSuffix);
                    }
        ...
        private void ParseRoadLookFiles()
        {
            ...
            foreach (var roadLookFileName in worldDirectory.GetFiles("road_look"))
            {
                ...
                if (roadLookFile?.Entry == null)
                {
                    Logger.Instance.Warning($"Skipping unreadable road_look file 'def/world/{roadLookFileName}' (likely missing from a mod .scs part or the file list is out of date)");
                    continue;
                }
                ...
                foreach (var line in lines)
                {
                    ...
                    if (validLine)
                    {
                        if (key == "road_look")
                        {
                            var rawSuffix = value.Split('.').Last();
                            var cleanSuffix = rawSuffix.Trim().TrimEnd('{').Trim();
                            roadLook = new TsRoadLook(ScsToken.StringToToken(cleanSuffix));
                        }
        ...
        private void ParseFerryConnections()
        {
            ...
            foreach (var ferryConnectionFilePath in connectionDirectory.GetFilesByExtension("def/ferry/connection", ".sui", ".sii"))
            {
                ...
                if (ferryConnectionFile?.Entry == null)
                {
                    Logger.Instance.Warning($"Skipping unreadable ferry connection file '{ferryConnectionFilePath}' (likely missing from a mod .scs part or the file list is out of date)");
                    continue;
                }
                ...
                foreach (var line in lines)
                {
                    ...
                    if (validLine)
                    {
                        if (conn != null)
                        {
                            ...
                            {
                                if (int.TryParse(value, NumberStyles.Integer, CultureInfo.InvariantCulture, out var price))
                                    conn.Price = price;
                            }
                            else if (key == "time")
                            {
                                if (int.TryParse(value, NumberStyles.Integer, CultureInfo.InvariantCulture, out var time))
                                    conn.Time = time;
                            }
                        ...
                        if (key == "ferry_connection")
                        {
                            ...
                            {
                                StartPortToken = ScsToken.StringToToken(portIds[1].Trim()),
                                EndPortToken = ScsToken.StringToToken(portIds[2].Trim().TrimEnd('{').Trim())
                            };
                        }
                    }
        ...
        public void ExportInfo(ExportFlags exportFlags, string exportPath)
        {
            ...
            if (exportFlags.IsActive(ExportFlags.RouteAdvisor)) RouteAdvisorExporter.Export(this, exportPath);
            if (exportFlags.IsActive(ExportFlags.FerryList)) FerryExporter.Export(this, exportPath);
            if (exportFlags.IsActive(ExportFlags.Graph)) GraphExporter.Export(this, exportPath);
        }
        ...
        public void ExportCities(ExportFlags exportFlags, string path)
        {
            ...
            foreach (var city in Cities)
            {
                ...
                var preferredCityName = Localization.GetPreferredLocaleValue(city.City.LocalizationToken);
                if (!string.IsNullOrWhiteSpace(preferredCityName))
                {
                    cityJObj["Name"] = preferredCityName;
                }
                else if (!string.IsNullOrWhiteSpace(city.City.Name))
                {
                    cityJObj["Name"] = city.City.Name;
                }
                else
                {
                    cityJObj["Name"] = ScsToken.TokenToString(city.City.Token);
                }
                cityJObj["X"] = city.X;
                cityJObj["Y"] = city.Z;
        ...
        private Dictionary<int, TsCountry> _countriesById;

        /// <summary>Looks up a country by its numeric id (matches TsNode.ForwardCountryId/BackwardCountryId).</summary>
        public TsCountry GetCountryById(int countryId)
        {
            if (_countriesById == null)
            {
                _countriesById = new Dictionary<int, TsCountry>();
                foreach (var country in _countriesLookup.Values)
                {
                    if (!_countriesById.ContainsKey(country.CountryId)) _countriesById[country.CountryId] = country;
                }
            }
            return _countriesById.TryGetValue(countryId, out var c) ? c : null;
        }
        ...
        /// <summary>All road looks parsed from def/world/road_look*.sii (base game + mods).</summary>
        public IEnumerable<TsRoadLook> GetAllRoadLooks()
        {
            return _roadLookup.Values;
        }
        ...
        /// <summary>
        /// Every connection touching this port, from either side. Unlike <see cref="LookupFerryConnection"/>
        /// (which only matches the "start" side, so the renderer draws each line exactly once),
        /// this is meant for exporters that need a per-port connection list, e.g. FerryExporter.
        /// </summary>
        public List<TsFerryConnection> GetFerryConnectionsForPort(ulong ferryPortId)
        {
            return _ferryConnectionLookup
                .Where(item => item.StartPortToken == ferryPortId || item.EndPortToken == ferryPortId)
                .ToList();
        }
