    ...
    internal class OverlayImage
    {
        ...
        internal void Parse()
        {
            ...
            else
            {
                ...
                var isDx10 = ddpf.FourCc == MemoryHelper.MakeFourCc('D', 'X', '1', '0');
                pixelDataOffset = isDx10 ? 0x94 : 0x80;

                Format = isDx10
                    ? (DxgiFormat) MemoryHelper.ReadUInt32(_stream, 0x80)
                    : Dds.GetDXGIFormat(ddpf);
        ...
        private void ParseBc7(int pixelDataOffset) // BC7/BPTC format - see Bc7Decoder.cs for the block decoder
        {
            var fileOffset = pixelDataOffset;
            _pixelData = new Color8888[Width * Height];
            for (var y = 0; y < Height; y += 4)
            for (var x = 0; x < Width; x += 4)
            {
                var block = new Bc7Block
                {
                    lowBits = MemoryHelper.ReadUInt64(_stream, fileOffset),
                    highBits = MemoryHelper.ReadUInt64(_stream, fileOffset + 8)
                };
                var colors = block.Decode();

                for (var j = 0; j < 4; ++j)
                for (var i = 0; i < 4; ++i)
                {
                    var c = colors[i + j * 4];
                    var pos = (y + j) * Width + (x + i);
                    if (pos < _pixelData.Length)
                        _pixelData[pos] = new Color8888(c.a, c.r, c.g, c.b);
                }

                fileOffset += 16; // BC7 blocks are always 16 bytes
            }
        }
